...
function update_user_level($user_id){
    ...
    $condition['order_status'] = array('exp','not in (3,5)');

    $total_amount = M('order')->where($condition)->field('order_id,order_amount,pay_status')->sum('order_amount');
...
function sendSMS($mobile, $code)
{
    ...
    $resp = $SmsLogic->sendSms($mobile,$templateCode,$templateParam);
...
function orderStatusDesc($order_id = 0, $order = array())
{
    ...
    if($order['order_status'] == 2){
        ...
        //分成已评价，待评价
        if($order['order_id'] > 0 ){
            $comment = M('comment')->where(array('order_id'=>$order['order_id']))->find();
        }
       
        if($comment || $order['is_comment'] == 1){
            return 'COMMENT'; //'已评价',
        }else{
...
function update_pay_status($order_sn,$pay_status = 1)
{
    ...
    $user_id = M('order')->where(array('order_sn'=>$order_sn))->getField('user_id');
    // 购买商品付款才可以成为分销商
    if($distribut_condition == 1 || $distribut_condition == 0 ){
        M('users')->where(array("user_id" => $user_id ))->save(array('is_distribut'=>1));
    }
    // 购买(299分区)商品付款才可以成为分销商
    if($distribut_condition == 2){
        //判断 299 分区
        $order_id = M('order')->where(array('order_sn'=>$order_sn))->getField('order_id');
        $goods_ids =  M('order_goods')->field('goods_id')->where(array('order_id'=>$order_id))->select();
        //购买的goods_id
       
        for($i=0;$i<count($goods_ids);$i++){
            $goods_temp_id = $goods_ids[$i]['goods_id'];
            $cat_id1 = M('goods')->where(array('goods_id'=>$goods_temp_id))->getField('cat_id1');
            if($cat_id1 == 9){
                M('users')->where(array("user_id" => $user_id ))->save(array('is_distribut'=>1));
            }
        }
	...
	if(stripos($order_sn,'recharge') !== false){
		//用户在线充值
		$count = M('recharge')->where(array("order_sn"=>$order_sn,"pay_status" => 0))->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
    ...
    // 记录订单操作日志
    logOrder($order_id,'订单付款成功','付款成功',$order['user_id'],2);
    ...
    //给商家发送信息
    $logic = new \Home\Logic\StoreLogic();
    $logic->seng_msg_to_store($order);
