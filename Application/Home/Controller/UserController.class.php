...
class UserController extends BaseController {
    ...
    public function order_detail(){
        ...
        //更新支付名称
        if($order_info['pay_status'] ==1 && $order_info['pay_name'] == ''){
            if( $order_info['user_money'] > 0 ){
                M('order')->where(array('order_id'=>$id))->save(array('pay_name'=>'余额支付'));
            }
        }
    ...
    */
    ...
    public  function recharge(){
    	if(IS_POST){
	   	...
	   	$count2 = M('account_log')->where(array('user_id'=>$this->user_id))->count();
	   	$Page2 = new Page($count2,10);
	   	$consume_list = M('account_log')->where(array('user_id'=>$this->user_id))->limit($Page2->firstRow.','.$Page2->listRows)->select();
