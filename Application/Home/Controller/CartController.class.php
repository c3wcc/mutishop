...
class CartController extends BaseController {
    ...
    public function test(){
       ...
       // echo "604 ====   602";
        ...
        $distributLogic = new \Home\Logic\DistributLogic();
        $res =  $distributLogic->rebate_log($order); // 生成分成记录
    ...
    public function cart3(){
       ...
       // if(!$shipping_code) exit(json_encode(array('status'=>-4,'msg'=>'请选择物流信息','result'=>null))); // 返回结果状态
        ...
        $post_fee = $result['result']['shipping_price'];
        if(!$post_fee || $post_fee == null){
            $post_fee = 0;
        }
        $car_price = array(
            'postFee'      => $post_fee, // 物流费
    ...
    public function cart4(){
        ...
        {                       
            ...
            $master_order = M('order')->where("master_order_sn = '$master_order_sn'")->find();
             // 如果已经支付过的订单直接到订单详情页面. 不再进入支付页面
             if($master_order['pay_status'] == 1){
                $order_order_list = U("Home/User/order_list");
                header("Location: $order_order_list");
            }
