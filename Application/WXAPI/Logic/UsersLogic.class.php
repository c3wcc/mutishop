...
class UsersLogic extends RelationModel
{
    ...
    public function get_order_goods($order_id){
        ...
        $goods_list = M('order_goods')-> join('LEFT JOIN tp_goods ON tp_goods.goods_id = tp_order_goods.goods_id')->where(array('order_id'=>$order_id))->select();
...
//        $type = I('get.type',0);
     ...
     * 获取账户提现记录
     */
    public function get_withdrawlist($user_id,$type=0){

        $count = M('withdrawals')->where('user_id = '.$user_id)->count();
        $Page = new Page($count,16);
        $logs = M('withdrawals')->where('user_id = '.$user_id)->order('create_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
...
public function get_comment($user_id,$status=2,$p){
    ...
    public function update_info($user_id,$post=array()){
	    ...
	    public function add_Withdraw($user_id,$address_id=0,$data){
        $post = $data;
                

        if($post['money'] == '')
            return array('status'=>-1,'msg'=>'提现金额不能为空','result'=>'');
        
        if(!$post['bank_name'])
            return array('status'=>-1,'msg'=>'银行不能为空','result'=>'');
        if(!$post['account_bank'])
            return array('status'=>-1,'msg'=>'银行卡号不能为空','result'=>'');

		if(!$post['account_name'])
            return array('status'=>-1,'msg'=>'开户人姓名不能为空','result'=>'');
		
        $user = M('users')->where("user_id='{$user_id}'")->find();
		if($user['user_money']<$post['money']){
			return array('status'=>-1,'msg'=>'余额不足','result'=>'');
		}
		$nowmoney=$user['user_money']-$post['money'];
        //添加模式
        ...
        $address_id = M('withdrawals')->add($post);
        $row = M('users')->where("user_id='{$user_id}'")->save(array('user_money'=>$nowmoney));
    ...
    public function cancel_order($user_id,$order_id){
        ...
        if( $order['order_status'] == 3){
            return array('status'=>-1,'msg'=>'订单已取消','result'=>'');
        }
        //获取记录表信息
