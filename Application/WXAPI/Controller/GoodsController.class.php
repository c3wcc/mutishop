...
class GoodsController extends BaseController
{
    ...
    public function categoryBanner()
    {
        ...
        if ($position_id) {
            $banner = M('ad')->where(array('pid' => $position_id,'enabled'=>1))->find();
            if ($banner) {
    ...
    public function cgoodsList()
    {
        ...
        $sort_asc = 'asc' == $sort_asc ? 'desc' : 'asc';
    ...
    public function goodsList()
    {
        ...
        $filter_goods_id = M('goods')->where($condition)->where('cat_id2 in('.$cat_ids.') OR cat_id1 in('.$cat_ids.') OR cat_id3 in('.$cat_ids.') ')->cache(true)->getField('goods_id', true);
    ...
    public function search()
    {
        ...
        if (empty($q)) {
            exit(json_encode(array('status' => -1, 'msg' => '请输入关键词'), true));
        }
        $goodsLogic = new \Home\Logic\GoodsLogic(); // 前台商品操作逻辑类
    ...
    */
     ...
     *  获取商品的缩略图.
     */
    public function goodsThumImages()
    {
        $goods_id = I('goods_id');
        $width = I('width');
        $height = I('height');
    ...
    public function collectGoods()
    {
        ...
        if (0 == $count) {
            exit(json_encode(array('status' => 1, 'msg' => '收藏商品不存在', 'result' => array())));
        }
        //删除收藏商品
        if (1 == $type) {
        ...
        if ($count > 0) {
            exit(json_encode(array('status' => 1, 'msg' => '已收藏该商品', 'result' => array())));
        }
