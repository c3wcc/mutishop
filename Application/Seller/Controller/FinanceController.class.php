...
class FinanceController extends BaseController {
    ...
    public function add_edit_withdrawals()
    {
        ...
        $withdrawals_min = M('config')->where(array('name'=>'min','inc_type'=>'basic'))->getField('value');

         if(IS_POST)
         {            
            $post = I('post.');
            if($post['money'] > $withdrawals_max ){
                $this->error('对不起，您的申请提现金额已经超过您的可申请提现金额');    
            }
            if($post['money'] < $withdrawals_min ){
                $this->error('对不起，您的可申请提现金额小于最少提现额度');    
            }
