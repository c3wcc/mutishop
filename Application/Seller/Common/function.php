...
function get_spec($id){
	$res = M('spec_item')->where(array('spec_id'=>$id))->field('item')->select();
	foreach($res as $val){
...
function getMenuList() {
	$menu_list = array(
			'goods' => array('name' => '商品管理', 'icon'=>'fa-tasks', 'child' => array(
					...
					array('name' => '商品列表', 'act'=>'goodsList?goods_state=1', 'op'=>'Goods'),
					...
					//array('name' => '商品类型', 'act' => 'goodsTypeList', 'op' => 'Goods'),
