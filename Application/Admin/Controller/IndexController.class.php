...
class IndexController extends BaseController {
    ...
    public function changeTableVal(){  
            ...
            //如果是修改平台扣点
            if($table == 'goods_category' && $field == 'commission'){
                $this->change_commission($table,$id_name,$id_value,$field,$value);
            }
    }	    
     ...
     * 则检查，是不是大类，是则同步更改
     */
    public function change_commission($table,$id_name,$id_value,$field,$value){
        
        if($table == 'goods_category' && $field == 'commission'){

            $catagory = M('goods_category')->where("$id_name = $id_value")->find();
            //找出 子类
            $parent_id_path = $catagory['parent_id_path'];
            $con['parent_id_path'] = array('like', '%'.$parent_id_path.'%');
            M('goods_category')->where($con)->save(array('commission'=>$value));

            //处理 商店已经 存在 的品类
            M('store_bind_class')->where(array('class_1'=>$id_value))->save(array('commis_rate'=>$value));
            M('store_bind_class')->where(array('class_2'=>$id_value))->save(array('commis_rate'=>$value));
            M('store_bind_class')->where(array('class_3'=>$id_value))->save(array('commis_rate'=>$value));
        }
    }
