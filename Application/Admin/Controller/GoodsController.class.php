...
class GoodsController extends BaseController {
    ...
    public function delGoodsCategory(){
        ...
        $id = I('id/d');
        if($id == 9){
            $this->error("299分区不得删除",U('Admin/Goods/categoryList'));
            exit;
        }
    ...
    public function examine(){
        $goods_id = I('id');
        if(!$goods_id){
            $this->error("商品ID不能为空!!!",U('Admin/Goods/examineGoodsList'));
        }
        $goods_state = I('goods_state');
        if(!$goods_state){
            $goods_state = 0;
        }
        M('Goods')->where(array('goods_id'=>$goods_id))->save(array('goods_state'=>$goods_state));

        $this->success("审核成功!!!",U('Admin/Goods/examineGoodsList'));
     ...
     *  商品列表
     */
    public function examineGoodsList(){      
        $GoodsLogic = new GoodsLogic();        
    ...
    public function ajaxGoodsList(){            
        ...
        if($key_word)
        {
            $con['goods_name'] = array('like',"%$key_word%") ; 
        ...
        $good_state =  $_GET['good_state'];
      
        if($good_state !== NULL )
        {         
            $con['goods_state'] = 0 ; // 审核商品
        }   
       
        $model = M('Goods');
        $count = $model->where($where)->where($con)->count();
        ...
        */
        ...
        $con['deleted'] = 0;
        $goodsList = $model->where($where)->where($con)->order($order_str)->limit($Page->firstRow.','.$Page->listRows)->select();
        ...
        if($good_state !== NULL )
        {         
            $this->display('ajaxExamineGoodsList');
        }  else{
            $this->display();       
        }
    ...
    public function ajaxGetSpecSelect(){
        ...
        $GoodsLogic = new GoodsLogic();
        //$_GET['spec_type'] =  13;
        $specList = D('Spec')->where("cat_id3 = ".$_GET['cat_id3'])->order('`order` desc')->select();
    ...
    public function del_goods_images()
    {
        ...
        //$res = M('goods_images')->where(array('image_url'=>$image_url))->delete();
        
        $goods_images = M('goods_images')->where(array('image_url'=>$image_url))->select();
