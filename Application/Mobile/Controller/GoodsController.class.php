...
class GoodsController extends MobileBaseController {
    ...
    public function goodsList(){
        ...
        $filter_goods_id = M('goods')->where(" goods_state = 1 and is_on_sale=1 and deleted = 0 and cat_id{$goodsCate['level']} = $id")->cache(true)->getField("goods_id",true);      
...
//        $show = $page->show();
    ...
    public function search(){
    	...
    	$goodsLogic = new \Home\Logic\GoodsLogic(); // 前台商品操作逻辑类    	     
    	$filter_goods_id = M('goods')->where(" goods_state = 1 and is_on_sale=1 and deleted = 0 and goods_name like '%{$q}%'  ")->cache(true)->getField("goods_id",true);
