...
class StoreController extends Controller {
	...
	public function _initialize() {
		$store_id = I('store_id');
		if(empty($store_id)){
			$this->error('参数错误,店铺系列号不能为空',U('Home/Index/index'));
		}
		$store = M('store')->where(array('store_id'=>$store_id))->find();
		if($store){
			if($store['store_state'] == 0){
				$this->error('该店铺不存在或者已关闭', U('Home/Index/index'));
			}
	...
	public function index(){
		//热门商品排行
		$hot_goods = M('goods')->field('goods_content',true)->where(array('store_id'=>$this->store['store_id'],'deleted'=>0 ))->order('sales_sum desc')->limit(10)->select();
		//新品
		$new_goods = M('goods')->field('goods_content',true)->where(array('store_id'=>$this->store['store_id'],'is_new'=>1,'deleted'=>0))->order('goods_id desc')->limit(10)->select();
		//推荐商品
		$recomend_goods = M('goods')->field('goods_content',true)->where(array('store_id'=>$this->store['store_id'],'is_recommend'=>1,'deleted'=>0))->order('goods_id desc')->limit(10)->select();
		//所有商品
		$total_goods = M('goods')->where(array('store_id'=>$this->store['store_id'],'is_on_sale'=>1,'deleted'=>0))->count();
	...
	public function goods_list(){
		...
		$sort = I('sort', 'desc');
		$keywords = I('keywords');
		$map = array('store_id' => $this->store['store_id'], 'is_on_sale' => 1 ,'deleted'=>0);
		$cat_name = "全部商品";
		if ($cat_id > 0) {
		...
		if ($count > 0) {
			$goods_list = M('goods')->where(" deleted= 0 and goods_id in (" . implode(',', $filter_goods_id) . ")")->order("$key $sort")->page($p,$page_count)->select();
		}
