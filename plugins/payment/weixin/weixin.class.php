...
{    
    ...
    public function  __construct() {   
        ...
        $config_value['appid'] = "wxfaa0361fbbdccabd";
        $config_value['mchid'] = "1501164731";
        $config_value['key'] = "ssHT2018scvIpapI033014w45Q12Ejjj";
    ...
    {       
            ...
            $name = $order['consignee'] ? $order['consignee'] : "充值" ;
            $input->SetBody("琅百广告（".$name."）的订单：".$order['order_id']); // 商品描述
            ...
            file_put_contents('pay.txt', $json);
    ...
    function getJSAPI($order){
      ...
      $openId = session('user.open_id');
        ...
        //②、统一下单
        $input = new WxPayUnifiedOrder();
        $input->SetBody("支付订单：".$order['order_id']);
        $input->SetAttach("weixin");
        $input->SetOut_trade_no($order['order_id'].time());
